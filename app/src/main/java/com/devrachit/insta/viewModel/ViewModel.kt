...
class LCViewModel @Inject constructor(
    ...
    val userNameValidationInProgress = mutableStateOf(false)
    ...
    suspend fun validateUserName(userName: String): Boolean {
        ...
        val taken = userNameAlreadyTaken(userName)?: false
        println("taken: $taken")
    ...
    suspend fun userNameAlreadyTaken(userName: String): Boolean? {
        return suspendCoroutine { continuation ->
            db.collection(USER_NODE).get().addOnSuccessListener { querySnapshot ->
                ...
                for (document in querySnapshot) {
                    ...
                    if (!taken){
                        signUp(emailData,"123456",userName)
                    }
                }
                println("taken: $taken")
